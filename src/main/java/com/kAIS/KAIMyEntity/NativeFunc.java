    ...
    private void Init() {
        try {
            if (isWindows) {
                ...
                LoadLibrary(new File(Minecraft.getInstance().gameDir.getAbsolutePath(), "KAIMyEntitySaba.dll"));//WIN32
            }
            if (isLinux && !isAndroid) {
                KAIMyEntity.logger.info("Linux Env Detected!");
                LoadLibrary(new File(Minecraft.getInstance().gameDir.getAbsolutePath(), "KAIMyEntitySaba.so"));//Linux
            }
            if (isLinux && isAndroid) {
